...
if [ ! -f "${REPO_ROOT}/blender_manifest.toml" ] || [ ! -f "${REPO_ROOT}/__init__.py" ]; then
    printf 'This does not look like the Slide Rotate extension root: %s\n' "$REPO_ROOT" >&2
    exit 1
fi

mkdir -p "$EXT_ROOT"
if [ -e "$TARGET" ] || [ -L "$TARGET" ]; then
    printf 'Replacing existing Slide Rotate install: %s\n' "$TARGET"
    rm -rf "$TARGET"
fi
ln -s "$REPO_ROOT" "$TARGET"

printf 'Linked:\n  %s\n→ %s\n\n' "$TARGET" "$REPO_ROOT"
printf 'Enable Slide Rotate, then use its sidebar Reload button after edits.\n'
